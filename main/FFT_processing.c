...
static float samples[SAMPLE_COUNT * 2];  // [Real, Imag, Real, Imag...]
static float magnitudes[SAMPLE_COUNT / 2];
...
void process_adc_sound(uint8_t* digi_sound_buf, float silence_offset) {
    ...
    dsps_fft2r_fc32(samples, SAMPLE_COUNT);
    dsps_bit_rev_fc32(samples, SAMPLE_COUNT);

    // Analyze Frequencies
    float max_mag = 0;
    int peak_idx = 0;
    for (int i = 2; i < SAMPLE_COUNT / 2; i++) {  // Skip bin 0 & 1 (DC noise)
...
void frequency_to_LED_position1(uint8_t* digi_sound_buf, uint8_t* led_color_buf,
    ...
    memset(led_color_buf, 0, LED_BUFFER_SIZE);
...
void frequency_to_LED_position2(uint8_t* digi_sound_buf, uint8_t* led_color_buf,
    ...
    for (int led_idx = 0; led_idx < LED_COUNT; led_idx += led_per_freq) {
        uint32_t red = 0;
        uint32_t green = 0;
        uint32_t blue = 0;
        uint32_t freq_idx = led_idx * freq_count / LED_COUNT;
        uint32_t hue = freq_idx * 360 / freq_count;
        uint32_t value = magnitudes[freq_idx];
