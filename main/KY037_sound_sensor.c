...
static void sound_event_task(void *arg) {
  ...
  for (;;) {
    if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
      ESP_LOGW(TAG, "GPIO[%" PRIu32 "] Triggered: Sound Threshold Exceeded!",
               io_num);
    }
  }
}
...
void setup_adc_DO(void) {
  ...
  gpio_isr_handler_add(SOUND_DIGITAL_PIN, DO_sound_isr_handler,
                       (void *)SOUND_DIGITAL_PIN);
}
...
void setup_adc_AO(adc_continuous_handle_t *i_handle) {
  ...
  adc_continuous_handle_cfg_t adc_config = {
      ...
      .conv_frame_size =
          READ_LEN *
          SOC_ADC_DIGI_DATA_BYTES_PER_CONV, // 2 bytes per sample (12-bit)
  };
...
float get_silence_offset(adc_continuous_handle_t adc_handle, uint8_t *buf,
                         size_t size) {
...
void app_main(void) {
  ...
  float silence_offset = get_silence_offset(
      adc_handle, result_buf, READ_LEN * SOC_ADC_DIGI_RESULT_BYTES);
  ...
  while (1) {
    ...
    if (adc_continuous_read(adc_handle, result_buf,
                            READ_LEN * SOC_ADC_DIGI_RESULT_BYTES, &ret_num,
                            0) == ESP_OK) {
      ...
      for (int i = 0; i < READ_LEN; i++) {
        ...
        adc_digi_output_data_t *p =
            (adc_digi_output_data_t
                 *)&result_buf[i * SOC_ADC_DIGI_RESULT_BYTES];
        uint32_t val = p->type2.data;
        sum += val;

        // ESP_LOGI(TAG, "read val: %.1f", (float)val);
        fft_input[i * 2 + 0] =
            (float)val - silence_offset; // Real (Remove DC Bias)
      ...
      if (max_mag > 1000) { // Detection threshold
        ESP_LOGI(
            TAG,
            "Average val: %.1d | Peak Frequency: %.1f Hz | Magnitude: %.1f",
